...
@implementation DBAPIManager

#pragma mark - override method
+ (void)initialize
{
    if (self != [DBAPIManager self]) {
        [self createTable];
    }
}
...
{
    __block BOOL res = NO;
    DBHelperManager *DBHelper = [DBHelperManager shareInstance];
...
- (BOOL)save
{
    ...
    [DBHelper.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        DEF_DEBUG(@"%@", res?[NSString stringWithFormat:@"%@插入成功",tableName]:[NSString stringWithFormat:@"%@插入失败",tableName]);
    }];
    return res;
}
...
{
    //判断是否是JKBaseModel的子类
    for (DBHelperManager *model in array) {
        if (![model isKindOfClass:[DBHelperManager class]]) {
            return NO;
        }
    }
    ...
    // 如果要支持事务
    [DBHelper.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (DBAPIManager *model in array)
        {
            ...
            DEF_DEBUG(@"%@", flag?[NSString stringWithFormat:@"%@插入成功",tableName]:[NSString stringWithFormat:@"%@插入失败",tableName]);
            if (!flag) {
                res = NO;
                *rollback = YES;
                return;
...
{
    for (DBAPIManager *model in array) {
        if (![model isKindOfClass:[DBAPIManager class]]) {
            return NO;
        }
    }
    __block BOOL res = YES;
...
+ (NSArray *)findAll
{
    ...
    [DBHelper.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            DBAPIManager *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
    ...
    //逆向遍历
    NSEnumerator *enumerator = [users reverseObjectEnumerator];
    users = (NSMutableArray*)[enumerator allObjects];
